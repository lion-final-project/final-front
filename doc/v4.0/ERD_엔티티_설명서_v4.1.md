...
| v4.1 | **DBMS 전환: MySQL 8.0 → PostgreSQL 16 + PostGIS 3.4**. `DATETIME`→`TIMESTAMPTZ`, `TINYINT(1)`→`BOOLEAN`, `AUTO_INCREMENT`→`GENERATED ALWAYS AS IDENTITY`, 인라인 `ENUM`→커스텀 타입(`CREATE TYPE`), 위경도 컬럼→`GEOGRAPHY(POINT,4326)` + GIST 인덱스, `FULLTEXT`→`GIN(pg_trgm)`, `ON UPDATE CURRENT_TIMESTAMP`→`set_updated_at()` 트리거 |
...
| store_order_id | BIGINT | FK → store_orders, NOT NULL | 마트별 주문 ID | 1 |
...
| cart_id | BIGINT | FK → carts, NOT NULL | 장바구니 ID | 1 |
| product_id | BIGINT | FK → products, NOT NULL | 상품 ID | 10 |
...
| status | store_status | NOT NULL, DEFAULT 'PENDING' | 배달원 상태 | APPROVED |
...
| store_order_id | BIGINT | FK → store_orders, NOT NULL, UNIQUE | 마트별 주문 ID (1:1) | 1 |
...
| store_order_id | BIGINT | FK → store_orders, NULL | 생성된 주문 ID | 15 |
...
| store_order_id | BIGINT | FK → store_orders, NOT NULL, UNIQUE | 마트주문 ID (1:1) | 1 |
...
| store_order_id | BIGINT | FK → store_orders, NOT NULL | 마트별 주문 ID | 10 |
...
| store_order_id | BIGINT | FK → store_orders, NULL | 관련 주문번호 | 10 |
