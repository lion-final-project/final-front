# ON DELETE 정책 정의서 v2.0

> 작성일: 2026-01-29
> 대상: ERD 엔티티 설명서 v2.0 (통합 승인 구조 반영)
...
| payment_methods | user_id | **RESTRICT** | 빌링키 관리 필요 |
...
| approvals | approved_by | **SET NULL** | 관리자 변경/퇴사 시 승인 이력 보존 |
...
| approvals | store_id | **RESTRICT** | 승인 이력 보존 |
...
| approvals | rider_id | **RESTRICT** | 승인 이력 보존 |
...
| 자식 테이블 | FK 컬럼 | ON DELETE | 사유 |
|------------|---------|-----------|------|
| approval_documents | approval_id | **CASCADE** | 승인 이력 삭제 시 서류도 삭제 |

> **참고**: `approvals`의 FK (부모 측)

| 부모 테이블 | FK 컬럼 | ON DELETE | 사유 |
|------------|---------|-----------|------|
| stores | store_id | **RESTRICT** | 마트 삭제 시 승인 이력 보존 |
| riders | rider_id | **RESTRICT** | 배달원 삭제 시 승인 이력 보존 |
| users | approved_by | **SET NULL** | 관리자 변경/퇴사 시 이력 보존 |

> **원칙**: 승인 이력은 물리 삭제 금지. 감사 추적(Audit Trail)을 위해 영구 보존.

### approval_documents (승인 서류)

자식 테이블 없음. `approvals` 삭제 시 CASCADE로 함께 삭제.
...
| 45 | **approvals** | **store_id** | **stores** | **RESTRICT** | **승인 이력 보존** |
| 46 | **approvals** | **rider_id** | **riders** | **RESTRICT** | **승인 이력 보존** |
| 47 | **approvals** | **approved_by** | **users** | **SET NULL** | **관리자 변경 대응** |
| 48 | **approval_documents** | **approval_id** | **approvals** | **CASCADE** | **종속 데이터** |
| 49 | notifications | user_id | users | RESTRICT | Soft Delete |
| 50 | reports | *(다형성)* | users/stores/riders | *(앱 레벨)* | 다형성 FK, DB 제약 불가 |
| 51 | inquiries | user_id | users | RESTRICT | 문의 이력 |
| 52 | inquiries | answered_by | users | SET NULL | 관리자 변경 대응 |
| 53 | inquiry_attachments | inquiry_id | inquiries | CASCADE | 종속 데이터 |
| 54 | notices | author_id | users | SET NULL | 관리자 변경 대응 |
| 55 | promotion_products | promotion_id | promotions | CASCADE | 종속 데이터 |
| 56 | promotion_products | product_id | products | CASCADE | 상품 삭제 시 매핑 제거 |
...
| 정책 | 건수 | 비율 | 적용 대상 |
|------|------|------|----------|
| **RESTRICT** | 31건 | 55% | 핵심 비즈니스 데이터 보호 |
| **CASCADE** | 17건 | 30% | 부모에 종속된 데이터 |
| **SET NULL** | 6건 | 11% | 참조 끊어져도 데이터 보존 |
| **앱 레벨** | 2건 | 4% | 다형성 FK (DB 제약 불가) |
| **합계** | **56건** | 100% | |
...
| 변경 사항 | v1.0 | v2.0 |
|----------|------|------|
| 승인 테이블 | store_approvals + rider_approvals (분리) | **approvals** (통합, applicant_type으로 구분) |
| 승인 서류 | approval_documents (store_id/rider_id 다형성 FK) | **approval_documents** (approval_id FK → approvals) |
| stores 서류 필드 | 인라인 (각종 서류 이미지) | **approval_documents로 이관** |
| riders 서류 필드 | id_card_image (인라인) | **approval_documents로 이관** |
| FK 총 건수 | 58건 | **56건** (통합으로 감소) |
| 다형성 FK | 3건 (approvals, settlements, reports) | **2건** (settlements, reports) — approvals는 CHECK 제약으로 해결 |
| 승인 상태 | 4단계 (PENDING, APPROVED, REJECTED, HELD) | **7단계** (+DOCUMENT_REVIEW, DOCUMENT_INCOMPLETE, REVIEWING) |
...
### 해결된 다형성 FK: approvals

v1.0에서 `approval_documents`의 `store_id`/`rider_id` 다형성 FK를 v2.0에서 CHECK 제약으로 해결:

```sql
-- approvals 테이블 CHECK 제약
CHECK (
...
- `store_id` → FK → stores (DB 레벨 제약 가능)
- `rider_id` → FK → riders (DB 레벨 제약 가능)
...
ALTER TABLE social_logins
  ADD CONSTRAINT fk_social_logins_user
  FOREIGN KEY (user_id) REFERENCES users(user_id)
  ON DELETE RESTRICT ON UPDATE CASCADE;
...
ALTER TABLE payment_refunds
  ADD CONSTRAINT fk_payment_refunds_payment
  FOREIGN KEY (payment_id) REFERENCES payments(payment_id)
  ON DELETE RESTRICT ON UPDATE CASCADE,
  ADD CONSTRAINT fk_payment_refunds_store_order
  FOREIGN KEY (store_order_id) REFERENCES store_order(store_order_id)
  ON DELETE RESTRICT ON UPDATE CASCADE;
...
ALTER TABLE rider_locations
  ADD CONSTRAINT fk_rider_locations_rider
  FOREIGN KEY (rider_id) REFERENCES riders(rider_id)
  ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT fk_rider_locations_delivery
  FOREIGN KEY (delivery_id) REFERENCES deliveries(delivery_id)
  ON DELETE CASCADE ON UPDATE CASCADE;

-- delivery_photos
ALTER TABLE delivery_photos
  ADD CONSTRAINT fk_delivery_photos_delivery
  FOREIGN KEY (delivery_id) REFERENCES deliveries(delivery_id)
  ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE subscription_products
  ADD CONSTRAINT fk_subscription_products_store
  FOREIGN KEY (store_id) REFERENCES stores(store_id)
  ON DELETE RESTRICT ON UPDATE CASCADE;

-- subscription_day_of_week
ALTER TABLE subscription_day_of_week
  ADD CONSTRAINT fk_subscription_dow_product
  FOREIGN KEY (subscription_product_id) REFERENCES subscription_products(subscription_product_id)
  ON DELETE CASCADE ON UPDATE CASCADE;

-- subscription_product_items
ALTER TABLE subscription_product_items
  ADD CONSTRAINT fk_sub_product_items_sub_product
  FOREIGN KEY (subscription_product_id) REFERENCES subscription_products(subscription_product_id)
  ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT fk_sub_product_items_product
  FOREIGN KEY (product_id) REFERENCES products(product_id)
  ON DELETE RESTRICT ON UPDATE CASCADE;
...
ALTER TABLE subscription_items
  ADD CONSTRAINT fk_subscription_items_subscription
  FOREIGN KEY (subscription_id) REFERENCES subscriptions(subscription_id)
  ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT fk_subscription_items_product
  FOREIGN KEY (product_id) REFERENCES products(product_id)
  ON DELETE RESTRICT ON UPDATE CASCADE;
...
ALTER TABLE subscription_history
  ...
  FOREIGN KEY (subscription_id) REFERENCES subscriptions(subscription_id)
  ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT fk_subscription_history_store_order
  FOREIGN KEY (store_order_id) REFERENCES store_order(store_order_id)
  ON DELETE SET NULL ON UPDATE CASCADE;
...
-- settlements: target_type + target_id 다형성 FK — 앱 레벨 검증
-- DB 제약 없음
...
-- ============================================================
-- 10. 승인 관리 모듈 (3.10) — v2.0 통합 구조
-- ============================================================

-- approvals (통합 승인)
ALTER TABLE approvals
  ADD CONSTRAINT fk_approvals_store
  FOREIGN KEY (store_id) REFERENCES stores(store_id)
  ON DELETE RESTRICT ON UPDATE CASCADE,
  ADD CONSTRAINT fk_approvals_rider
  FOREIGN KEY (rider_id) REFERENCES riders(rider_id)
  ON DELETE RESTRICT ON UPDATE CASCADE,
  ADD CONSTRAINT fk_approvals_admin
  FOREIGN KEY (approved_by) REFERENCES users(user_id)
  ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT chk_approvals_applicant CHECK (
    (applicant_type = 'STORE' AND store_id IS NOT NULL AND rider_id IS NULL) OR
    (applicant_type = 'RIDER' AND store_id IS NULL AND rider_id IS NOT NULL)
  );
...
ALTER TABLE approval_documents
  ADD CONSTRAINT fk_approval_docs_approval
...
-- reports: reporter/target 다형성 FK — 앱 레벨 검증
-- DB 제약 없음
