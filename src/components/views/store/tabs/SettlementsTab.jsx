...
const SettlementsTab = ({
  ...
  const platformFee = sales?.platformFee ?? 0;
  const refundAmount = sales?.refundAmount ?? 0;
  const settlementAmount = Math.max(0, totalSales - platformFee - refundAmount);
  const totalOrderCount = sales?.totalOrderCount ?? 0;
  const regularOrderCount = sales?.regularOrderCount ?? 0;
  const subscriptionOrderCount = sales?.subscriptionOrderCount ?? 0;
  const regularPct = totalOrderCount > 0 ? Math.round((regularOrderCount / totalOrderCount) * 100) : 0;
  const subscriptionPct = totalOrderCount > 0 ? Math.round((subscriptionOrderCount / totalOrderCount) * 100) : 0;
  const monthOverMonthRate = sales?.monthOverMonthRate ?? 0;
  const dayOverDayRate = sales?.dayOverDayRate ?? 0;
  const refundCount = sales?.refundCount ?? 0;
  const averageOrderAmount = sales?.averageOrderAmount ?? 0;
  const periodList = Array.isArray(periodOptions) && periodOptions.length > 0 ? periodOptions : ['2026년 1월', '2025년 12월', '2025년 11월', '2025년 10월'];

  const summaryCards = [
    { label: '총 결제 금액', value: formatCurrency(totalSales), sub: `${monthOverMonthRate >= 0 ? '↗' : '↘'}${monthOverMonthRate}% 전월 대비`, color: '#1e293b', subColor: monthOverMonthRate >= 0 ? '#10b981' : '#ef4444' },
    { label: '플랫폼 수수료 합계', value: `-${formatCurrency(platformFee)}`, sub: '고정 8% 플랫폼 수수료 적용', color: '#ef4444', subColor: '#94a3b8' },
    { label: '환불/취소 금액', value: `-${formatCurrency(refundAmount)}`, sub: refundCount > 0 ? `${refundCount}건의 취소 내역 반영` : '해당없음', color: '#94a3b8', subColor: '#94a3b8' },
    { label: '최종 정산 예정 금액', value: formatCurrency(settlementAmount), sub: '🗓️ 익월 정산일 지급 예정', color: '#ffffff', subColor: '#ffffff', highlight: true },
  ];

  const statRows = [
    { label: '총 주문 건수', value: `${totalOrderCount} 건`, icon: '🛍️' },
    { label: '환불 건수', value: `${refundCount} 건`, icon: '🔄' },
    { label: '평균 주문 금액', value: formatCurrency(averageOrderAmount), icon: '💳' },
    { label: '매출 증감률', value: `${dayOverDayRate >= 0 ? '+' : ''}${dayOverDayRate}%`, icon: '📈' },
  ];
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
    ...
    {salesLoading && (
      <div style={{ background: 'white', padding: '48px', borderRadius: '24px', border: '1px solid #f1f5f9', textAlign: 'center', color: '#64748b', fontWeight: '600' }}>
        매출 정보를 불러오는 중...
      </div>
    )}
    {salesError && !salesLoading && (
      <div style={{ background: '#fef2f2', padding: '24px', borderRadius: '24px', border: '1px solid #fecaca', color: '#b91c1c', fontWeight: '600' }}>
        {salesError}
      </div>
    )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '32px' }}>
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', fontSize: '14px', fontWeight: '700' }}>
            ...
            <div><span style={{ color: '#3b82f6' }}>● 일반 {regularPct}%</span><span style={{ color: '#8b5cf6', marginLeft: '16px' }}>● 구독 {subscriptionPct}%</span></div>
          </div>
          <div style={{ height: '12px', width: '100%', backgroundColor: '#f1f5f9', borderRadius: '6px', overflow: 'hidden', display: 'flex' }}>
            <div style={{ width: `${regularPct}%`, height: '100%', background: 'linear-gradient(90deg, #3b82f6, #60a5fa)' }}></div>
            <div style={{ width: `${subscriptionPct}%`, height: '100%', background: 'linear-gradient(90deg, #8b5cf6, #a78bfa)' }}></div>
          </div>
        </div>
        <div style={{ backgroundColor: '#f8fafc', padding: '20px', borderRadius: '20px', border: '1px solid #f1f5f9' }}>
          ...
          <div style={{ fontSize: '24px', fontWeight: '900' }}>{regularOrderCount} <span style={{ fontSize: '14px', fontWeight: '600', color: '#94a3b8' }}>건</span></div>
        </div>
        <div style={{ backgroundColor: '#f5f3ff', padding: '20px', borderRadius: '20px', border: '1px solid #ede9fe' }}>
          ...
          <div style={{ fontSize: '24px', fontWeight: '900', color: '#8b5cf6' }}>{subscriptionOrderCount} <span style={{ fontSize: '14px', fontWeight: '600', color: '#a78bfa' }}>건</span></div>
        </div>
      </div>
    </div>
    ...
    <div style={{ background: 'white', padding: '32px', borderRadius: '24px', border: '1px solid #f1f5f9' }}>
      ...
      <p style={{ fontSize: '13px', color: '#94a3b8', marginBottom: '16px' }}>주문별 상세는 주문 내역 탭에서 확인할 수 있습니다.</p>
