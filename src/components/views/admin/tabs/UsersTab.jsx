...
const UsersTab = ({
  users, userSearch, setUserSearch, expandedUserId, setExpandedUserId,
  currentPage, itemsPerPage, setCurrentPage, setSelectedRecord
}) => {
  const userStats = [
      ...
      <div style={{ backgroundColor: '#1e293b', padding: '32px', borderRadius: '24px', border: '1px solid #334155' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <input type="text" placeholder="고객명, 이메일, 연락처 검색..." value={userSearch} onChange={(e) => setUserSearch(e.target.value)}
            ...
            <tbody>
              {filteredUsers.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((user, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #334155', fontSize: '14px' }}>
                  ...
                  <td style={{ padding: '16px' }}>
                    <button onClick={() => setSelectedRecord(user)}
